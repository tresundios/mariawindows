<?php
/**
 * Class to Increase Memory and Time
 *
 * @package Support functions.
 */

/**
 * Class WPBC_Action_Scheduler_Compatibility
 */
class WPBC_Action_Scheduler_Compatibility {
	...
	public static function raise_memory_limit() {
		...
		if ( -1 === $filtered_limit_int || ( $filtered_limit_int > $wp_max_limit_int && $filtered_limit_int > $current_limit_int ) ) {
			if ( false !== @ini_set( 'memory_limit', $filtered_limit ) ) {  // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged, Squiz.PHP.DiscouragedFunctions.Discouraged
				return $filtered_limit;
			} else {
				return false;
			}
		} elseif ( -1 === $wp_max_limit_int || $wp_max_limit_int > $current_limit_int ) {
			if ( false !== @ini_set( 'memory_limit', $wp_max_limit ) ) {  // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged, Squiz.PHP.DiscouragedFunctions.Discouraged
				return $wp_max_limit;
			} else {
				return false;
			}
	...
	public static function raise_time_limit( $limit = 0 ) {
		...
		if (
			...
			|| ( $max_execution_time >= $limit && 0 !== $limit )
		) {
			return;
		}
		...
		if ( function_exists( 'wc_set_time_limit' ) ) {
			...
			@set_time_limit( $limit ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged, Squiz.PHP.DiscouragedFunctions.Discouraged
		}
	}
}
