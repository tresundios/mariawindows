...
function softaculous_pro_license(){
	...
	if(isset($_POST['save_sp_license'])){
		...
		if(is_array($resp)){
			...
			$spro_error['resp_invalid'] = __('The response was malformed', 'softaculous-pro').'<br />'.var_export($resp, true);
			return softaculous_pro_license_T();
		}
		...
		<?php
			...
			<tr>			
				...
				<td align="left">
					...
					<input name="save_sp_license" class="button button-primary" value="'.__('Update License', 'softaculous-pro').'" type="submit" />';
					...
					if(!empty($softaculous_pro['license'])){
						...
						echo '<div style="margin-top:10px;">'.__('License Status : ', 'softaculous-pro').(empty($softaculous_pro['license']['status_txt']) ? 'N.A.' : $softaculous_pro['license']['status_txt']).' &nbsp; &nbsp; &nbsp; 
						'.($softaculous_pro['license']['expires'] <= date('Ymd') ? __('License Expires : ', 'softaculous-pro').' <span style="color:red">'.$expires.'</span>' : (empty($softaculous_pro['license']['has_plid']) ? __('License Expires : ', 'softaculous-pro').' '.$expires : '')).'
						</div>';
					}
