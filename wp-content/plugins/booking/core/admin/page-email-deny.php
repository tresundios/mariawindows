...
if ( ! defined( 'WPBC_EMAIL_DENY_PREFIX' ) )   define( 'WPBC_EMAIL_DENY_PREFIX',  'booking_email_' ); // Its defined in api-emails.php file & its same for all emails, here its used only for easy coding...

if ( ! defined( 'WPBC_EMAIL_DENY_ID' ) )       define( 'WPBC_EMAIL_DENY_ID',      'deny' );      /* Define Name of Email Template.   
                                                                                                                   Note. Prefix "booking_email_" defined in api-emails.php file. 
                                                                                                                   Full name of option is - "booking_email_deny"
...
function wpbc_import6_is_old_email_deny_exist() {
...
function wpbc_import6_get_old_email_deny_data() {
    
    $default_values = array();
    if ( wpbc_import6_is_old_email_deny_exist() ) {
...
function wpbc_import6_is_new_email_deny_exist() {
    
    $is_data_exist = get_bk_option( WPBC_EMAIL_DENY_PREFIX . WPBC_EMAIL_DENY_ID );           // ''booking_email_' - defined in api-emails.php  file.
...
function wpbc_import6_email__deny__get_fields_array_for_activation() {
    
           
    if ( ! wpbc_import6_is_new_email_deny_exist() ) {                      // New Email Template NOT exist
    
        // Import
        $old_data = wpbc_import6_get_old_email_deny_data();
...
//            $old_data['content'] = html_entity_decode( $old_data['content'] );
        ...
        $default_fields_in_settings = $mail_api->get_default_values() ;         // Get default fields values, from settings - defined in function init_settings_fields {class WPBC_Emails_API_Deny}

        $fields_values = wp_parse_args( $old_data, $default_fields_in_settings );
        
        return array( WPBC_EMAIL_DENY_PREFIX . WPBC_EMAIL_DENY_ID => $fields_values );
        
        /*
        Array ( 
                [booking_email_deny] => Array
                                                (
                                                    [enabled] => On
        ...
        return array( WPBC_EMAIL_DENY_PREFIX . WPBC_EMAIL_DENY_ID => array() );
    }
}
// </editor-fold>
...
/** Email   F i e l d s  */
class WPBC_Emails_API_Deny extends WPBC_Emails_API  {                       // O v e r r i d i n g     "WPBC_Emails_API"     ClASS
...
class WPBC_Settings_Page_Email_Deny extends WPBC_Page_Structure {
    ...
    public function mail_api( $selected_email_name ='',  $init_fields_values = array() ){
        ...
        if ( $this->email_settings_api === false ) {
            $this->email_settings_api = new WPBC_Emails_API_Deny( $selected_email_name , $init_fields_values );    
        }
...
//                    );
    ...
    private function enqueue_js(){                                               // $page_tag, $active_page_tab, $active_page_subtab ) {
        ...
        $js_script .= " jQuery('select[name=\"deny_template_file\"]').on( 'change', function(){    
                                if ( jQuery('select[name=\"deny_template_file\"] option:selected').val() == 'plain' ) {   
        ...
        $js_script .= "\n";                                                     //New Line
        $js_script .= " if ( jQuery('select[name=\"deny_template_file\"] option:selected').val() == 'plain' ) {   
                            jQuery('.template_colors').hide();                                    
                        } ";    
        ...
        $js_script .= "     if (  jQuery('#deny_to_name').val() == ''  ) {";
        $js_script .= "         jQuery('#deny_to_name').parent().append('<div class=\'updated\' style=\'border-left-color:#ffb900;padding:5px 10px;\'>". esc_js(__('If empty then title defined as WordPress', 'booking' ))."</div>')";
        $js_script .= "     }";
        $js_script .= "     if (  jQuery('#deny_from_name').val() == ''  ) {";
...
add_action('wpbc_menu_created',  array( new WPBC_Settings_Page_Email_Deny() , '__construct') );    // Executed after creation of Menu
 ...
 */
function wpbc__get_replace_shortcodes__email_deny( $booking_id, $bktype, $formdata ) { 
...
function wpbc_send_email_deny( $deny_id_str, $is_send_emeils, $denyreason = '' ) {
