...
if ( !class_exists('FileorganizerUpdateChecker_3_2', false) ):
 ...
 * @access public
 */
class FileorganizerUpdateChecker_3_2 {
	...
	public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = ''){
		...
		$this->upgraderStatus = new Fileorganizer_PucUpgraderStatus_3_2();
	 ...
	 * @return Fileorganizer_PucScheduler_3_2
	 */
	protected function createScheduler($checkPeriod) {
		return new Fileorganizer_PucScheduler_3_2($this, $checkPeriod);
	}
	...
	public function requestInfo($queryArgs = array()){
		...
		if ( !is_wp_error($status) ){
			$pluginInfo = FileorganizerInfo_3_2::fromJson($result['body']);
			if ( $pluginInfo !== null ) {
	 ...
	 *
	 * @uses FileorganizerUpdateChecker::requestInfo()
	 *
	 * @return FileorganizerUpdate_3_2 An instance of FileorganizerUpdate, or NULL when no updates are available.
	 */
	public function requestUpdate(){
		...
		$update = FileorganizerUpdate_3_2::fromFileorganizerInfo($pluginInfo);
	 ...
	 *
	 * @return FileorganizerUpdate_3_2|null
	 */
	...
	public function getUpdateState() {
		...
		if ( isset($state, $state->update) && is_object($state->update) ) {
			$state->update = FileorganizerUpdate_3_2::fromObject($state->update);
		}
		return $state;
	}
	...
	private function setUpdateState($state) {
		if ( isset($state->update) && is_object($state->update) && method_exists($state->update, 'toStdClass') ) {
			$update = $state->update; /** @var FileorganizerUpdate_3_2 $update */
	 ...
	 * @param FileorganizerUpdate_3_2 $updateToAdd
	 * @return StdClass
	 */
	 ...
	 * successfully, the first argument passed will be an instance of  FileorganizerInfo. Otherwise, 
	 * it will be NULL. The second argument will be the corresponding return value of 
	 * wp_remote_get (see WP docs for details).
	 *  
	 * The callback function should return a new or modified instance of FileorganizerInfo or NULL.
	 * 
	 * @uses add_filter() This method is a convenience wrapper for add_filter().
	 * 
	 * @param callable $callback
	...
	public function initDebugBarPanel() {
		...
		if ( class_exists('Debug_Bar', false) && file_exists($debugBarPlugin) ) {
			...
			$this->debugBarPlugin = new Fileorganizer_PucDebugBarPlugin_3_2($this);
		}
	}
...
if ( !class_exists('FileorganizerInfo_3_2', false) ):
...
class FileorganizerInfo_3_2 {
	...
	/**
	 * Create a new instance of FileorganizerInfo from JSON-encoded plugin info 
	 * returned by an external update API.
	 * 
	 * @param string $json Valid JSON string representing plugin info.
	 * @return FileorganizerInfo_3_2|null New instance of FileorganizerInfo, or NULL on error.
	 */
...
if ( !class_exists('FileorganizerUpdate_3_2', false) ):
...
class FileorganizerUpdate_3_2 {
	...
	/**
	 * Create a new instance of FileorganizerUpdate from its JSON-encoded representation.
	 * 
	 * @param string $json
	 * @return FileorganizerUpdate_3_2|null
	 */
	public static function fromJson($json){
		...
		$pluginInfo = FileorganizerInfo_3_2::fromJson($json);
		if ( $pluginInfo != null ) {
			return self::fromFileorganizerInfo($pluginInfo);
		} else {
			return null;
		}
	}
	...
	/**
	 * Create a new instance of FileorganizerUpdate based on an instance of FileorganizerInfo.
	 * Basically, this just copies a subset of fields from one object to another.
	 * 
	 * @param FileorganizerInfo_3_2 $info
	 * @return FileorganizerUpdate_3_2
	 */
	...
	/**
	 * Create a new instance of FileorganizerUpdate by copying the necessary fields from 
	 * another object.
	 *  
	 * @param StdClass|FileorganizerInfo_3_2|FileorganizerUpdate_3_2 $object The source object.
	 * @return FileorganizerUpdate_3_2 The new copy.
	 */
	 ...
	 * a FileorganizerUpdate. Useful for serialization and caching, as it avoids
...
if ( !class_exists('Fileorganizer_PucScheduler_3_2', false) ):

/**
 * The scheduler decides when and how often to check for updates.
 * It calls @see FileorganizerUpdateChecker::checkForUpdates() to perform the actual checks.
 *
 * @version 3.2
 */
class Fileorganizer_PucScheduler_3_2 {
	public $checkPeriod = 12; //How often to check for updates (in hours).
	public $throttleRedundantChecks = false; //Check less often if we already know that an update is available.
	public $throttledCheckPeriod = 72;

	/**
	 * @var FileorganizerUpdateChecker_3_2
	 */
	 ...
	 *
	 * @param FileorganizerUpdateChecker_3_2 $updateChecker
...
if ( !class_exists('Fileorganizer_PucUpgraderStatus_3_2', false) ):
 ...
 */
class Fileorganizer_PucUpgraderStatus_3_2 {
...
if ( !class_exists('Fileorganizer_PucFactory', false) ):

/**
 * A factory that builds instances of other classes from this library.
 *
 * When multiple versions of the same class have been loaded (e.g. FileorganizerUpdateChecker 1.2
 * and 1.3), this factory will always use the latest available version. Register class
 * versions by calling {@link Fileorganizer_PucFactory::addVersion()}.
 *
 * At the moment it can only build instances of the FileorganizerUpdateChecker class. Other classes
 * are intended mainly for internal use and refer directly to specific implementations. If you
 * want to instantiate one of them anyway, you can use {@link Fileorganizer_PucFactory::getLatestClassVersion()}
 * to get the class name and then create it with <code>new $class(...)</code>.
 */
class Fileorganizer_PucFactory {
	protected static $classVersions = array();
	protected static $sorted = false;

	/**
	 * Create a new instance of FileorganizerUpdateChecker.
	 *
	 * @see FileorganizerUpdateChecker::__construct()
	 *
	 * @param $metadataUrl
	 * @param $pluginFile
	 * @param string $slug
	 ...
	 *
	 * @param string $generalClass Class name without version numbers, e.g. 'FileorganizerUpdateChecker'.
	 * @param string $versionedClass Actual class name, e.g. 'FileorganizerUpdateChecker_1_2'.
...
Fileorganizer_PucFactory::addVersion('FileorganizerUpdateChecker', 'FileorganizerUpdateChecker_3_2', '3.2');
Fileorganizer_PucFactory::addVersion('FileorganizerUpdate', 'FileorganizerUpdate_3_2', '3.2');
Fileorganizer_PucFactory::addVersion('FileorganizerInfo', 'FileorganizerInfo_3_2', '3.2');
Fileorganizer_PucFactory::addVersion('Fileorganizer_PucGitHubChecker', 'Fileorganizer_PucGitHubChecker_3_2', '3.2');
