...
if ( ! defined( 'WPBC_EMAIL_APPROVED_PREFIX' ) )   define( 'WPBC_EMAIL_APPROVED_PREFIX',  'booking_email_' ); // Its defined in api-emails.php file & its same for all emails, here its used only for easy coding...

if ( ! defined( 'WPBC_EMAIL_APPROVED_ID' ) )       define( 'WPBC_EMAIL_APPROVED_ID',      'approved' );      /* Define Name of Email Template.   
                                                                                                                   Note. Prefix "booking_email_" defined in api-emails.php file. 
                                                                                                                   Full name of option is - "booking_email_approved"
...
function wpbc_import6_is_old_email_approved_exist() {
    
    $is_old_data_exist = get_bk_option( 'booking_email_approval_content' );
...
function wpbc_import6_get_old_email_approved_data() {
    
    $default_values = array();
    if ( wpbc_import6_is_old_email_approved_exist() ) {
        $default_values['enabled']  = get_bk_option( 'booking_is_email_approval_adress' );
        $default_values['copy_to_admin']       = get_bk_option( 'booking_is_email_approval_send_copy_to_admin' );          //Parse it to  Name and Email 
        $default_values['from']     = get_bk_option( 'booking_email_approval_adress' );     //Parse it to  Name and Email, relpace [visitoremail]
        $default_values['subject']  = get_bk_option( 'booking_email_approval_subject' );
        $default_values['content']  = get_bk_option( 'booking_email_approval_content' );
...
function wpbc_import6_is_new_email_approved_exist() {
    
    $is_data_exist = get_bk_option( WPBC_EMAIL_APPROVED_PREFIX . WPBC_EMAIL_APPROVED_ID );           // ''booking_email_' - defined in api-emails.php  file.
...
function wpbc_import6_email__approved__get_fields_array_for_activation() {
    
           
    if ( ! wpbc_import6_is_new_email_approved_exist() ) {                      // New Email Template NOT exist
    
        // Import
        $old_data = wpbc_import6_get_old_email_approved_data();
...
//            $old_data['content'] = html_entity_decode( $old_data['content'] );
        ...
        $default_fields_in_settings = $mail_api->get_default_values() ;         // Get default fields values, from settings - defined in function init_settings_fields {class WPBC_Emails_API_Approved}

        $fields_values = wp_parse_args( $old_data, $default_fields_in_settings );
        
        return array( WPBC_EMAIL_APPROVED_PREFIX . WPBC_EMAIL_APPROVED_ID => $fields_values );
        
        /*
        Array ( 
                [booking_email_approved] => Array
                                                (
                                                    [enabled] => On
        ...
        return array( WPBC_EMAIL_APPROVED_PREFIX . WPBC_EMAIL_APPROVED_ID => array() );
    }
}
// </editor-fold>
...
/** Email   F i e l d s  */
class WPBC_Emails_API_Approved extends WPBC_Emails_API  {                       // O v e r r i d i n g     "WPBC_Emails_API"     ClASS
...
//FixIn: 8.1.2.17 - END
        ...
        $this->fields['from'] = array(  
                                    ...
                                    //, 'placeholder' => ''
                                    , 'title'       => ''
                                    , 'description' => __('Email Address', 'booking') . '. ' . __('Required', 'booking') . '. '  . __('Email Address will be used in the FROM field of response to Visitor.', 'booking')    // FixIn: 8.1.2.17.
                                    , 'description_tag' => ''
                                    , 'css'         => 'width:100%'
                                    , 'group'       => 'general'
                                    , 'tr_class'    => ''
	    ...
	    $this->fields['subject_before'] = array(
												...
												, 'html' =>  '<tr><td colspan="2">'
															.'<div style="margin: 0 0 15px;font-weight: 600;">'
															 .'<label class="wpbc-form-email-subject" for="' . WPBC_EMAIL_APPROVED_ID . '_subject">' .
															 		__( 'Subject', 'booking' )
															 . '</label></div>'
							);
        $this->fields['subject'] = array(
                                      'type'        => 'text'
                                    , 'default'     => __( 'Your booking has been approved', 'booking' )
                                    //, 'placeholder' => ''
	    ...
	    if ( class_exists( 'wpdev_bk_personal' ) ) {
		    /* translators: 1: ... */
		    $email_content = sprintf( __( 'Your reservation %1$s for: %2$s has been approved.%3$sYou can edit the booking on this page: %4$s Thank you, %5$s', 'booking' ), '[bookingtype]', '[dates]', '<br/><br/>[content]<br/><br/>', '[visitorbookingediturl]<br/><br/>', $blg_title . '<br/>[siteurl]' );
	    } else {
		    /* translators: 1: ... */
		    $email_content = sprintf( __( 'Your booking %1$s for: %2$s has been approved.%3$sThank you, %4$s', 'booking' ), '[bookingtype]', '[dates]', '<br/><br/>[content]<br/><br/>', $blg_title . '<br/>[siteurl]' );
	    }
	    ...
	    $this->fields['content_before'] = array(
												...
												'html'  => '<tr><td colspan="2" style="padding-bottom:0;">'
															   ...
															   . '<label class="wpbc-form-email-content" for="' . WPBC_EMAIL_APPROVED_ID . '_content">' .
																   __( 'Content', 'booking' )
															   . '</label></div>'
											);
...
class WPBC_Settings_Page_Email_Approved extends WPBC_Page_Structure {
    ...
    public function mail_api( $selected_email_name ='',  $init_fields_values = array() ){
        ...
        if ( $this->email_settings_api === false ) {
            $this->email_settings_api = new WPBC_Emails_API_Approved( $selected_email_name , $init_fields_values );    
        }
...
//                    );
        ...
        $is_data_exist = get_bk_option( WPBC_EMAIL_NEW_ADMIN_PREFIX . WPBC_EMAIL_APPROVED_ID );
        ...
        $subtabs['approved'] = array( 
                            'type' => 'subtab'                                  // Required| Possible values:  'subtab' | 'separator' | 'button' | 'goto-link' | 'html'
        					, 'title' => '<span>' . esc_html__( 'Approved', 'booking' ) . $sufix . '</span>' 					// Title of TAB
                            , 'page_title' => __('Emails Settings', 'booking')  .  ' - <span>' . esc_html__( 'Approved', 'booking' ) . $sufix . '</span>'  // Title of Page.
                            , 'hint' => __('Customization of email template, which is sent to Visitor after approval of booking' ,'booking')   //FixIn: 8.1.2.17.1
                            , 'link' => ''                                      // link
                            , 'position' => ''                                  // 'left'  ||  'right'  ||  ''
                            , 'css_classes' => ''                               // CSS class(es)
							...
							, 'font_icon' => 'wpbc_icn_done_all'
							, 'show_checked_icon' 		=> true
							, 'checked_data' 			=> WPBC_EMAIL_NEW_ADMIN_PREFIX . WPBC_EMAIL_APPROVED_ID		// This is where we get content
                        );
        ...
        <span class="metabox-holder">
            ...
            <form name="<?php echo esc_attr( $submit_form_name ); ?>" id="<?php echo esc_attr( $submit_form_name ); ?>" action="" method="post" autocomplete="off">
                ...
                <div class="metabox-holder">
					<?php
                        wpbc_open_meta_box_section( $submit_form_name . 'general', __('Email is sent to Visitor after Approval of booking.', 'booking')   );	//FixIn: 8.1.2.17.1
    ...
    private function enqueue_js(){
        ...
        $js_script .= " jQuery('select[name=\"approved_template_file\"]').on( 'change', function(){    
                                if ( jQuery('select[name=\"approved_template_file\"] option:selected').val() == 'plain' ) {   
        ...
        $js_script .= "\n";                                                     //New Line
        $js_script .= " if ( jQuery('select[name=\"approved_template_file\"] option:selected').val() == 'plain' ) {   
                            jQuery('.template_colors').hide();                                    
                        } ";    
        ...
        $js_script .= "     if (  jQuery('#approved_to_name').val() == ''  ) {";
        $js_script .= "         jQuery('#approved_to_name').parent().append('<div class=\'updated\' style=\'border-left-color:#ffb900;padding:5px 10px;\'>". esc_js(__('If empty then title defined as WordPress', 'booking' ))."</div>')";
        $js_script .= "     }";
        $js_script .= "     if (  jQuery('#approved_from_name').val() == ''  ) {";
        $js_script .= "         jQuery('#approved_from_name').parent().append('<div class=\'updated\' style=\'border-left-color:#ffb900;padding:5px 10px;\'>". esc_js(__('If empty then title defined as WordPress', 'booking' ))."</div>')";
        $js_script .= "     }";
        $js_script .= "  }); ";

        // FixIn: 8.1.2.17.
		$js_script .= " jQuery('#approved_copy_to_admin').on( 'change', function(){ ";
        $js_script .= "     if ( jQuery('#approved_copy_to_admin').is(':checked') ) {";
...
add_action('wpbc_menu_created',  array( new WPBC_Settings_Page_Email_Approved() , '__construct') );    // Executed after creation of Menu
 ...
 */
function wpbc__get_replace_shortcodes__email_approved( $booking_id, $bktype, $formdata ) { 
	...
	$replace['remote_ip'] = wpbc_get_user_ip();
...
//											) );
	...
	$google_calendar_link = wpbc_booking_do_action__get_google_calendar_link( array(
																						...
																						'form_show'   => $booking_form_show_array['content'],
																						'dates_short' => array(
	...
	$replace['add_to_google_cal_button'] = '<a href="' . esc_attr( $replace['add_to_google_cal_url'] ) . '" target="_blank" rel="nofollow">' . esc_html__( 'Add to Google Calendar', 'booking' ) . '</a>';
...
function wpbc_send_email_approved( $approved_id_str, $is_send_emeils, $denyreason = '' ) {
	...
	foreach ( $result as $res ) {
		...
		// Check for ability to change HASH during approving booking    //FixIn: 7.0.1.35.
		make_bk_action( 'booking_aproved', $res, '' );
